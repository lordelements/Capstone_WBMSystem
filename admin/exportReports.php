...
// }
    ...
    $reports = [
        ['Report ID', 'Position', 'Place' , 'Complainant' , 'Complainee' , 'Witness 1' , 'Witness 2' , 'Narrative', 'Date and Time']
    ];
    ...
    if (mysqli_num_rows($res) > 0) {
        foreach ($res as $row) {
            $reportid++;
            $reports = array_merge($reports, array(array($reportid, $row['incident'], $row['incidentplace'],
            $row['complainant'], $row['complainee'], $row['witness1'], $row['witness2'], $row['narrative'], $row['dateandtime'])));
        }
    }

    $xlsx = Shuchkin\SimpleXLSXGen::fromArray( $reports );
    $xlsx->downloadAs('reports.xlsx');
