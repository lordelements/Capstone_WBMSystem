...
<html dir="ltr" lang="en-US">
	...
	<!-- <?php	
		$brgyid = $_POST['brgyid'];
		$sql = "SELECT brgy_logo FROM tbl_brgyinfo WHERE brgyid = $brgyid LIMIT 1";
		$result = $cn->query($sql);

		
			while($row = mysqli_fetch_row($result))
				{       
					$query = mysqli_query($cn, "SELECT brgy_logo FROM tbl_brgyinfo WHERE brgyid = $brgyid LIMIT 1");
					$disp = mysqli_fetch_assoc($query);

					echo "<div class='row bg-overlay bg-overlay-dark3'>";
					echo "<h1>".$row['province']."</h1>";
					echo "<img  class='img-fluid' src='uploaded_brgy/".$row['brgy_logo']." 'width=100%; height=500px;  border-radius=5px; alt='Image'>";
